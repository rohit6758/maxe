...
const start = c.indexOf('const THEME_DECORATIONS = {');
const end = c.indexOf('export default function ProSettingsModal');
...
`;
  ...
  c = c.substring(0, start) + NEW_DECORATIONS + c.substring(end);
  fs.writeFileSync('src/components/ProSettingsModal.jsx', c);
  console.log('patched THEME_DECORATIONS');
} else {
