...
// 3. Replace the profile view section
      ...
      {!isEditing ? (
        <div className="card p-4 relative overflow-hidden" style={{
          ...
          {userProfile?.is_premium && profileEffects?.banner === 'gradient' && (
            <div className="absolute top-0 left-0 right-0 h-16 opacity-30" style={{background: 'linear-gradient(90deg, var(--theme-primary), var(--theme-accent, var(--theme-ring)))'}} />
          )}
...
const elseIdx = c.indexOf('      ) : (', returnIdx);

if (returnIdx !== -1 && elseIdx !== -1) {
  c = c.substring(0, returnIdx) + NEW_RETURN_START + c.substring(elseIdx + 11);
  
  // Also append the Pro modal at the end, right before the last closing div
  const proModal = `\n      {/* Pro Settings Modal */}\n      {showProModal && <ProSettingsModal isOpen={showProModal} onClose={() => setShowProModal(false)} />}\n\n`;
  c = c.replace(
    '{showAvatarPopup && (',
    proModal + '{showAvatarPopup && ('
  );
